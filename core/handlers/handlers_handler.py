...
async def handlers_handler(app, message):
    ...
    while True:
        ...
        if len(handlers) == 0:
            ...
            await message.reply(text=respond, disable_web_page_preview=True)
            return
        ...
        respond = '\n'.join(respond)
        if not bot_message:
            bot_message = await message.reply(text=respond, disable_web_page_preview=True)
        else:
            await bot_message.edit(text=respond, disable_web_page_preview=True)
        ...
        if answer is None:
            await bot_message.edit(
                text=respond + app.get_core_string('message_is_outdated'),
