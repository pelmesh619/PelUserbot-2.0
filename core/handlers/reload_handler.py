...
async def reload_handler(app, message):
    if len(message.command) > 1:
        ...
        await message.reply_text(app.get_core_string('reload_enter_module_name'))
        return

    module = app.get_module(module_name)
    if module is None:
        await message.reply_text(
            app.get_core_string('reload_module_not_found', module_name=module_name)
        )
        return
    ...
    respond = app.get_core_string(
        ...
        added_handlers=app.get_string(
            ...
            added_handler_form=app.get_core_string_form('handler_form', added_handlers),
            added_form=app.get_core_string_form('added_form', added_handlers),
        ...
        removed_handlers=app.get_string(
            ...
            removed_handler_form=app.get_core_string_form('handler_form', removed_handlers),
            removed_form=app.get_core_string_form('removed_form', removed_handlers),
            _module_id='core.handlers'
        ) if removed_handlers else '',
        reloaded_handlers=app.get_core_string(
            'reloaded_handlers_template',
            reloaded_handlers_count=reloaded_handlers,
            reloaded_handler_form=app.get_core_string_form('handler_form', reloaded_handlers),
            reloaded_form=app.get_core_string_form('reloaded_form', reloaded_handlers),
